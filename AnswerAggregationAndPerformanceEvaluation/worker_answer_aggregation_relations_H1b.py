...
def get_all_worker_annotations(filepath, assignment_status_to_include_in_analysis):
    ...
    return all_worker_annotations, filter_step2_results
...
all_worker_annotations, filter_step2_results = get_all_worker_annotations(filepath, assignment_status_to_include_in_analysis)
...
def performance_analysis_H1b(filter_step2_results, data_prepared_for_aggregation):
    ...
    for paragraph_name, paragraph_value in data_prepared_for_aggregation.items():
        ...
        for combination in worker_combinations:
            ...
            for assignment_id in combination:
                ...
                nr_of_tries = list({k:v["nr_of_tries"] for (k,v) in filter_step2_results[assignment_id]["tried_parts"].items()}.values())
                ...
                for try_id, try_annotations in all_answers_all_tries.items():
                    # loop trough this workers annotation tries for all questions
                    for annotation_value in try_annotations:
                        # for each question, loop through the answers given by the worker
                        if annotation_value == {}:
            ...
            h1b_data.append([nr_of_tries_median_combined_AVG, nr_of_tries_sum_combined_AVG, total_nr_of_empty_and_wrong_answers_combined_AVG, f1_nodes, f1_nodes_and_types, f1_TOTAL])
...
h1b_data = performance_analysis_H1b(filter_step2_results, data_prepared_for_aggregation)

# initialize csv file to save results from H1b worker answers
csv_filename = 'H1b_scores_relations.csv'

with open(os.path.join('AnswerAggregationAndPerformanceEvaluation', csv_filename), 'w', newline='') as file:
    writer = csv.writer(file)
    
    # write header
    writer.writerow(["nr_of_tries_median","nr_of_tries_sum","total_nr_of_empty_and_wrong_answers","f1_nodes","f1_nodes_and_types","f1_TOTAL"])
